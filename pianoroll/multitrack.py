...
class MultiTrack(object):
    ...
    def get_merged_pianoroll(self, track_indices=None, mode='max',
        ...
        mode : {'max', 'sum', 'any'}
            ...
            to 'sum'.
        binarized : bool
            If True, return a binarized merged piano-roll. Otherwise, return the
            raw merged piano-rolls. Default to False.
        threshold : int
    ...
    def merge_tracks(self, track_indices=None, mode='sum', remove_merged=False,
                     program=0, is_drum=False, name='merged', binarized=True,
                     threshold=0.0):
        """
        ...
        binarized : bool
            True to binarize the merged piano-rolls. False to use the raw merged
            piano-rolls. Default to False.
        threshold : int
            Threshold to binarize the collected piano-rolls. Only effective when
            ``binarized`` is True. Default to zero.
        """
    ...
    def parse_midi(self, filepath, mode='sum', clipped=True, algorithm='normal',
                   beat_start_time=None, binarized=False, threshold=0.0):
        """
        Parse a MIDI file
        ...
        self.parse_pretty_midi(pm, mode=mode, clipped=clipped,
                               algorithm=algorithm, binarized=False,
                               threshold=0.0, beat_start_time=beat_start_time)
    ...
    def parse_pretty_midi(self, pm, mode='sum', clipped=True,
        ...
        # Parse tempo array
        self.tempo = np.empty((self.beat_resolution * num_beat,))
        if not tc_times.size:
            ...
            for idx, tempo in enumerate(tempi[:-1]):
                ...
                start_idx = start * self.beat_resolution
                end_idx = end * self.beat_resolution
                self.tempo[start_idx:end*end_idx] = tempo
                start = end
